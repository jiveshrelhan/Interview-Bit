	...
	public int lengthOfLongestSubstring(String A) {
		...
		for (int i = 0; i < A.length(); i++) {
			...
			if (map.containsKey(c)) {
				// For every new character, we look for it in the already visited characters. If
				// the character has already been visited and is part of the current substring
				// with non-repeating characters
				left = Math.max(map.get(c) + 1, left);
			}
			map.put(c, i);
			length = i - left + 1;
			// System.out.println(A.substring(left, i + 1));
